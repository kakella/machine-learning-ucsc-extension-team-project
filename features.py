...
# main() is renamed to extract()
# modified original main() and save() functions
...
def compute_features_bryce(filepath):
...
def save(features, ndigits, saveName):

    # Should be done already, just to be sure.
 #   features.sort_index(axis=0, inplace=True)
#    features.sort_index(axis=1, inplace=True)

    features.to_csv(saveName, float_format='%.{}e'.format(ndigits))
...
if __name__ == "__main__":
    try:
        opts, args = getopt.getopt(sys.argv[1:], "m:", ["mp3="])
